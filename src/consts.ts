// Place any global data in this file.
// You can import this data from anywhere in your site by using the `import` keyword.

export const SITE_TITLE = "Zheng's Homepage"
export const SITE_DESCRIPTION = 'Personal website of Zheng Ma'
export const SITE_URL = 'https://zheng-homepage.netlify.app'
export const PUBLICATIONS = [
  {
    id: 'BINO2025',
    year: 2025,
    title:
      'Bayesian Inversion with Neural Operator (BINO) for modeling subdiffusion: Forward and inverse problems',
    author: 'Yan, Xiong-Bin and Xu, Zhi-Qin John and Ma, Zheng',
    journal: 'Journal of Computational and Applied Mathematics',
    pages: 116191,
    volume: 454,
    status: 'published',
    tags: ['Inverse Problems', 'Neural Operator', 'Subdiffusion'],
  },
  {
    id: 'APCON2024',
    year: 2024,
    title:
      'Capturing the diffusive behavior of the multiscale linear transport equations by Asymptotic-Preserving Convolutional DeepONets',
    author: 'Wu, Keke and Yan, Xiong-Bin and Jin, Shi and Ma, Zheng',
    journal: 'Computer Methods in Applied Mechanics and Engineering',
    pages: 116531,
    volume: 418,
    status: 'published',
    tags: ['DeepONet', 'Asymptotic-Preserving', 'Multiscale'],
  },
  {
    id: 'APNN2024',
    year: 2024,
    title:
      'Asymptotic-Preserving Neural Networks for Multiscale Kinetic Equations',
    author: 'Jin, Shi and Ma, Zheng and Wu, Keke',
    journal: 'Communications in Computational Physics',
    pages: 693,
    volume: 35,
    status: 'Preprint',
    tags: ['Neural Network', 'Asymptotic-Preserving', 'Kinetic Equation'],
  },
]
